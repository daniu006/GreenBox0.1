...
import { PlantRepository, Plant, CreatePlantData, PLANT_CATEGORIES } from './plant.repository';
...
export class PlantService {
  ...
  async getById(id: number): Promise<Plant> {
    ...
    if (!plant) throw new NotFoundException(`Planta con ID ${id} no encontrada`);
    return plant;
  }
  ...
  async update(id: number, data: Partial<CreatePlantData>): Promise<Plant> {
    ...
    if (!exists) throw new NotFoundException(`Planta con id ${id} no encontrada`);
