...
export class SaveHistoryUseCase {
  private readonly logger = new Logger(SaveHistoryUseCase.name);
  ...
  async execute(userPlantId: number, type: HistoryType): Promise<History> {
    ...
    // 2. Obtener lecturas según el tipo de historial
    ...
    // 3. Calcular promedios del período
    ...
    // 4. Calcular salud estimada del período
