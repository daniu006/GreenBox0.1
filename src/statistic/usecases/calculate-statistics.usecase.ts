...
export class CalculateStatisticsUseCase {
  private readonly logger = new Logger(CalculateStatisticsUseCase.name);
  ...
  async execute(userPlantId: number): Promise<Statistic | null> {
    // 1. Obtener userPlant con parámetros de la planta
    ...
    // 2. Lecturas de los últimos 7 días
    ...
    // 4. Calcular salud estimada — solo en backend, no en frontend
    ...
    // 5. Upsert — si ya existe la semana la actualiza, si no la crea
  ...
  // Cálculo de salud — ponderado por importancia de cada parámetro
